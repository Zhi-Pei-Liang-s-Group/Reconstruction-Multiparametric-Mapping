...
img_recon       = reshape(forward_model_windowed(h,x_ref,conv_sz) * sqrt(L1*L2*L3),[L1,L2,L3]);
...
function y = forward_model_windowed(x,x_ref,conv_sz)
...
% zero-padding
h_zp       = ktrunc(F3_x2k(imresize_ktrunc(F3_k2x(h),size(h),1,0)),[L1,L2,L3]);
