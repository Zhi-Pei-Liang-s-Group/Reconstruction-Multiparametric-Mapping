...
def build_2dbrain_dataset(opt, log, train, transform=None):
    """ resize -> crop -> to_tensor -> norm(-1,1) """
    if train:
        fn = opt.dataset_dir_train
    else:
        fn = opt.dataset_dir_val
    ...
    dataset = _build_2dbrain_dataset(fn, log, transform=transform)
    log.info(f"[Dataset] Built Imagenet dataset {fn=}, size={len(dataset)}!")
    return dataset
