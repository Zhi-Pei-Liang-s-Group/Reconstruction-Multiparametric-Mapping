...
%       Modified by Yibo Zhao @ UIUC, 2019/10/12:
%           Print more information when the data to load does not exist in the file.
%
%   Example:
...
%% ------ load variables ------
    ...
    if ~exist(fileName,'file') % if still doesn't exist
        fprintf('The specified file %s doesn''t exist.\nCannot load any data.\n',fileName);
        loadStruct = [];
        return;
    end
        ...
        for nn = 1:length(dataName) % for each data
            if ~existInBase(dataName{nn}) % if it is not loaded yet
                ...
                try
                    ...
                    fprintf('\b\b\b, done! (%ds)\n',round(toc));
                    fprintf('The specified variable %s doesn''t exist.\n',dataName{nn});
...
%#ok<*AGROW>
 ...
 % an old version (without parseInputs)
%{
loadStruct = myLoadVars(fileName,dataName,verbose) 
    ...
    assert(iscellstr(dataName),'Variable names must be strings.')
