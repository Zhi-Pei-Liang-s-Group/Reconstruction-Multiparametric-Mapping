...
%% ========================================================================
% KTRUNC zero-pad or truncate a k-space data. If doNorm, the intensity in
...
%       Modified by Yibo Zhao @ UIUC, 2018/12/22
%           Fix an mis-calculation of the normalization factor
%
%   Example:
%     temp  = phantom(256);
%     d_in  = F3_k2x(temp);
...
%--------------------------------------------------------------------------
    ...
    %% --- finalize ---
    if(doNorm == 1)&&(doPad == 0)&&(~islogical(dktc_in)) % if zero-filling, no need to normalize
        dktc_out = dktc_out*sqrt(prod(newdims)/prod(dims(1:numNewdim)));
    end
