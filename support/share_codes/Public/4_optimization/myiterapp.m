function y = myiterapp(op,afun,atype,afcnstr,x,varargin)
    ...
    switch lower(op)
        case 'mtimes'
            if (nargin >= 6) && isequal(varargin{end},'transp')
                error('Transp unsupported!');
            else
%                 y = afun(x);
                y = afun*x;
            end
        case 'mldivide'
            error('Preconditioner unsupported!');
        otherwise
    ...
    if ~iscolumn(y)
% @@ Yibo: in principle, y has to be a column vector. However...
